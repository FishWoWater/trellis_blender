...
class TrellisProperties(PropertyGroup):
    ...
    sparse_structure_sample_steps: IntProperty(
        name="ss sample steps",
        description="Number of sampling steps for sparse structure",
        default=12,
        min=1
    )
    sparse_structure_cfg_strength: FloatProperty(
        name="ss cfg strength",
        description="CFG strength for sparse structure",
        default=7.5,
        min=0.0
    )
    slat_sample_steps: IntProperty(
        name="slat sample steps",
        description="Number of sampling steps for SLAT",
        default=12,
        min=1
    )
    slat_cfg_strength: FloatProperty(
        name="slat cfg strength",
    ...
    texture_size: IntProperty(
        name="texture size",
        description="Size of the texture used for GLB",
        default=1024,
        min=64
    )
    texture_bake_mode: EnumProperty(
        name="tex bake mode",
        description="Mode for texture baking",
        items=[
            ('opt', "optimized", "Optimized texture baking"),
    ...
    task_id: StringProperty(
        name="Task ID",
        description="Current task ID for tracking conversion progress",
        default=""
    )
    show_parameters: BoolProperty(
        name="Show Parameters",
        description="Show/hide generation parameters",
        default=True
    )
...
class TRELLIS_PT_main_panel(Panel):
    ...
    def draw(self, context):
        ...
        row = params_box.row()
        row.prop(props, "show_parameters", text="Parameters", icon='TRIA_DOWN' if props.show_parameters else 'TRIA_RIGHT', emboss=False)
